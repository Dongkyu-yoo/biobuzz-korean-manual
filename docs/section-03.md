...
i. FIRST Dashboard를 통해 연간 등록 절차를 완료합니다.

ii. 연간 등록비를 납부합니다.

iii. 성인 2명이 Lead Coach 1/Lead Coach 2 역할에 지정되고 YPP 심사를 통과해야 합니다.

iv. 지역별 추가 Youth Protection 심사 요건을 완료합니다.

v. 모든 청소년 팀원을 FIRST Dashboard에 등록합니다.

B. **북미 외:**

i. FIRST Dashboard를 통해 연간 등록 절차를 완료하고,

ii. 프로그램 비용, 등록, 청소년 보호 심사에 관한 지역 Program Delivery Partner의 추가 요건을 완료합니다.

{% hint style="info" %}
지역 Program Delivery Partner는 드문 경우 지연 납부에 대해 사례별 예외를 적용할 수 있습니다. 연간 등록비를 납부하지 않은 팀은 공식 시즌 기록을 얻지 못하며 진출할 수 없습니다.
{% endhint %}
...
지역에 따라 다음 중 하나 이상이 요구될 수 있습니다.

A. FIRST Dashboard의 최신 완료 팀 명단

B. 지역 Program Delivery Partner의 팀원 등록 또는 동의서

C. MATCH 참가 시 Section 12에 맞게 제작된 ROBOT

D. 인쇄된 팀 PORTFOLIO(선택 사항)

{% hint style="info" %}
체크인 마감 전에 검사 또는 심사가 있을 수 있으므로 대회 일정을 확인하십시오. 모든 팀은 준비 정도와 관계없이 경기와 심사에 참여할 것을 권장하며, 필요하면 Program Delivery Partner와 다른 팀에 ROBOT 준비 도움을 요청하십시오.
{% endhint %}
...
STUDENT 팀원은 ROBOT, 사용될 모든 COMPONENT, OPERATOR CONSOLE을 제시합니다. INSPECTOR는 기본 안전 및 대회 요건을 확인하고 미충족 사항의 해결을 돕습니다. ROBOT 제작과 관련된 Qualification 및 Playoff MATCH 참가 자격에 대해서는 LRI가 최종 권한을 갖습니다.

규칙 준수를 입증하기 위해 검사 중 ROBOT을 켜고 활성화할 수 있습니다. 필요한 경우 INSPECTOR에게 알리고, 검사 구성에 저장 에너지(예: 늘어난 스프링)가 있다면 이를 알려 안전하게 협력해야 합니다.

{% hint style="info" %}
**Inspection은 모든 사항을 빠짐없이 확인하는 절차가 아닙니다.** INSPECTOR가 모든 부분을 확인하지 않더라도 Section 12의 규칙 정신을 따라야 합니다. ROBOT 제작 규칙을 STRATEGIC하게 우회하면 CIC를 준수하지 않는 것이며 완화 조치 대상이 될 수 있습니다. Inspection Checklist를 활용해 모든 이벤트 전에 자체 검사를 할 것을 강력히 권장합니다.
{% endhint %}
...
다음은 ROBOT의 크기, 합법성 또는 안전에 중대한 변화가 생기지 않는 한 일반적으로 재검사가 필요하지 않습니다.

A. 케이블 타이, 테이프, 리벳 등 체결부품의 추가·이동·제거

B. 라벨 또는 표시의 추가·이동·제거

C. 팀 SIGN의 추가·이동·교체

D. ROBOT 코드 수정

E. COMPONENT를 동일한 COMPONENT로 교체

F. MECHANISM을 크기·무게·재료가 동일한 MECHANISM으로 교체

G. 이미 검사받은 MECHANISM의 일부 조합으로 ROBOT을 추가·제거·재구성

{% hint style="info" %}
이벤트 중 ROBOT을 변경하는 모든 팀은 ROBOT 제작 규칙을 준수하기 위해 선의의 노력을 해야 합니다. 재검사 필요 여부가 불분명하면 LRI 또는 FIELD STAFF에게 확인하십시오.
{% endhint %}
